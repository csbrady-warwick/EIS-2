MODULE eis_header
  ...
  INTEGER(eis_error), PARAMETER :: eis_err_deck = 2**5 !< Error in deck
  INTEGER(eis_error), PARAMETER :: eis_err_not_found = 2**6 !< Name not found
  !> Malformed mathematical expression
  INTEGER(eis_error), PARAMETER :: eis_err_malformed = 2**7
  !> Incorrect number of parameters specified
  INTEGER(eis_error), PARAMETER :: eis_err_wrong_parameters = 2**8
  !> Specified expression is mathematically invalid (log of 0 etc.)
  INTEGER(eis_error), PARAMETER :: eis_err_maths_domain = 2**9
  !> Value is nonsensical in non-mathematical way
  INTEGER(eis_error), PARAMETER :: eis_err_bad_value = 2**10
  !> Stack has deferred elements that cannot be resolved
  INTEGER(eis_error), PARAMETER :: eis_err_has_deferred = 2**11
  !> Stack has emplaced elements that have not been resolved
  INTEGER(eis_error), PARAMETER :: eis_err_has_emplaced = 2**12
  !> Stack makes use of the "where" construct but the necessary "no-op"
  !> value is not tested when the stack is evaluated
  INTEGER(eis_error), PARAMETER :: eis_err_where = 2**13
  !> Attempted to subscript a constant as though it was a function
  INTEGER(eis_error), PARAMETER :: eis_err_bracketed_constant = 2**14
  !> Extra bracket in expression
  INTEGER(eis_error), PARAMETER :: eis_err_extra_bracket = 2**15
  !> Stack specified through interoperable interface is invalid
  INTEGER(eis_error), PARAMETER :: eis_err_bad_stack = 2**16
  !> Stack returned more results through interoperable interface than expected
  INTEGER(eis_error), PARAMETER :: eis_err_extra_results = 2**17
  !> String handler couldn't find a LUN when loading a file
  INTEGER(eis_error), PARAMETER :: eis_err_no_luns = 2**18
  !> String handler couldn't find a specified file
  INTEGER(eis_error), PARAMETER :: eis_err_no_file = 2**19
  !> String handler was given an invalid serialisation string
  INTEGER(eis_error), PARAMETER :: eis_err_malformed_file = 2**20
  !> Deck handler found an unmatched start and end block description
  INTEGER(eis_error), PARAMETER :: eis_err_mismatched_begin_end = 2**21
  !> Deck handler found a deck block that was deeper than permitted
  INTEGER(eis_error), PARAMETER :: eis_err_deck_too_deep = 2**22
  !> Deck handler found an unpermitted empty block
  INTEGER(eis_error), PARAMETER :: eis_err_deck_empty_block = 2**23
  !> Deck handler found an unpermitted key in the root block
  INTEGER(eis_error), PARAMETER :: eis_err_root_keys = 2**24
